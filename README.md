...
#### Ngrok (public URL, for testing from outside)

In Docker mode the app is not exposed via host ports; instead, `docker-compose.yml` runs an `ngrok` container that tunnels the `frontend` service.

1. Set your ngrok token in the repo root `.env`:
   - `NGROK_AUTHTOKEN=...`
   - Optional: `NGROK_DOMAIN=your-subdomain.ngrok-free.app` (a fixed custom domain)
2. Start everything:
   - `docker compose up -d --build`
3. Get your public URL:
   - `docker compose logs -f ngrok`

Email verification / password reset links are generated by the backend using `FRONTEND_PUBLIC_URL`. Docker compose will default it to `https://${NGROK_DOMAIN}` if provided; otherwise it falls back to a safe non-empty value.
